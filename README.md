...
Each alert contains the trader’s rank, total predictions in Cade’s current 24-hour leaderboard, and the exact number of trades completed during the trailing one-hour window aligned to the current sample. For example, an alert at `02:05:00 UTC` reports the counter delta for `01:05:00–02:05:00 UTC`, not a fixed `01:00–02:00` clock bucket:
